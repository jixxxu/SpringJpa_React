							...
							<fieldset>
								<div class="input-group">
									<select name="searchFilter" class="form-control mr-sm-1"
										...
										<option value="ename"
											${param.searchFilter == 'ename' ? 'selected' : ''}>
											사원 이름</option>
										<option value="empno"
											${param.searchFilter == 'empno' ? 'selected' : ''}>
											사원 번호</option>
										<option value="deptno"
											${param.searchFilter == 'deptno' ? 'selected' : ''}>
											부서 번호</option>
										<option value="job"
											${param.searchFilter == 'job' ? 'selected' : ''}>직급
										</option>
										<option value="mgr"
											${param.searchFilter == 'mgr' ? 'selected' : ''}>
											관리자</option>
									</select> <input type="search" name="searchQuery"
										class="form-control mr-sm-1" placeholder="검색어 입력"
										value="${param.searchQuery}">
									<button class="btn btn-outline-success" type="submit">
										검색</button>
									&nbsp; <a href="/search" class="btn btn-outline-info"> 목록
										이동 </a>
								</div>
							</fieldset>
						</form>
					...
					<tbody>
						<c:forEach var="employee" items="${employees}">
							<tr>
								...
								<td><fmt:formatNumber value="${employee.sal}"
										type="currency" /></td>
								<td><fmt:formatNumber value="${employee.comm}"
										type="currency" /></td>
				...
				<nav class="d-flex justify-content-center">
					<ul class="pagination">
						<li class="page-item ${page == 1 ? 'disabled' : ''}"><a
							href="/search?page=${page - 1}&searchFilter=${param.searchFilter}&searchQuery=${param.searchQuery}"
							class="page-link">이전</a></li>
						<c:forEach begin="1" end="${totalPages}" var="i">
							<li class="page-item ${i == page ? 'active' : ''}"><a
								href="/search?page=${i}&searchFilter=${param.searchFilter}&searchQuery=${param.searchQuery}"
